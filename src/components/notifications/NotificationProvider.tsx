...
const shouldKeepToastPersistent = (
  mode: NotificationDismissalMode,
): boolean => {
    ...
    for (const entry of activeNotifications) {
      ...
      const duration =
        baseDuration === null
          ? undefined
          : (baseDuration ??
            (shouldKeepToastPersistent(definition.dismissal) ? 120000 : 6500));
      const closeButton =
        definition.sonner?.closeButton ?? definition.dismissal !== "sticky";
      ...
      const showToast = (options: Parameters<typeof toast>[1]) => {
        ...
        if (variant && variant !== "default") {
          const possibleHandler = (toast as unknown as Record<string, unknown>)[
            variant
          ];
      ...
      if (definition.sonner?.important !== undefined) {
        (toastOptions as Record<string, unknown>).important =
          definition.sonner.important;
      }
