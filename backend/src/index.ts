...
async function fetchAndCacheCalendar(
  ...
  try {
    ...
    try {
      ...
      if (!isBackground) {
        console.log(
          `Successfully cached calendar data for URL: ${calendarUrl}`,
        );
      }
  ...
  try {
    ...
    if (!calendarUrl) {
      return res.status(400).json({
        ...
        message:
          "Please provide a calendar URL as a query parameter: ?url=your_calendar_url",
      });
    }
    ...
    console.log(
      `Fetching fresh data for URL: ${calendarUrl}${cachedData ? " (cache stale)" : " (no cache)"}`,
    );
...
app.get("/api/realization/:id", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `Successfully cached realization data for ID: ${realizationId}`,
    );
