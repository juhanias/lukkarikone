...
export class DateFormatUtils {
   ...
   */
  static calculateNonHiddenHours(
   ...
   */
  static formatDayViewDate(
    date: Date,
    events: ScheduleEvent[] = [],
    isEventHidden: (eventId: string) => boolean = () => false,
    showTotalHours: boolean = true,
  ): { dayWeek: string; fullDate: string } {
    const t = i18n.getFixedT(i18n.language, "schedule");

    // Convert Sunday (0) to be day 6, Monday (1) to be day 0, etc.
    const dayIndex = date.getDay() === 0 ? 6 : date.getDay() - 1;
    const dayNames = DateFormatUtils.getLocalizedDays();
    const monthNames = DateFormatUtils.getLocalizedMonths();
    ...
    const week = DateFormatUtils.getWeekNumber(date);
    const nonHiddenHours = DateFormatUtils.calculateNonHiddenHours(
      events,
      isEventHidden,
    );
    const hoursDisplay = DateFormatUtils.formatHoursDisplay(
      nonHiddenHours,
      showTotalHours,
    );
  ...
  static formatWeekHeader(startDate: Date, endDate: Date): string {
    const locale = DateFormatUtils.getCurrentLocale();
  ...
  static getDayNamesShort(includeWeekends: boolean = true): string[] {
    const dayNamesShort = DateFormatUtils.getLocalizedDaysShort();
